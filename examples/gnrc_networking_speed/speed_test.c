...
/* for speedtest purpose */
//xtimer_ticks32_t speedTimer;
uint32_t startTime;
uint32_t endTime;
                         ...
                         gnrc_netif_hdr_t *netif_hdr)
{
    ...
    if (icmpv6_hdr->type == ICMPV6_ECHO_REP) {
        ...
        rxPacketSize = gnrc_pkt_len(icmpv6);
        endTime = xtimer_now_usec();
        printf("\n----------------------\n");
        printf("IPv6 packet (TX packet size %d bytes and RX packet size %d bytes) travelled in %lu us\n",
                txPacketSize, rxPacketSize, endTime-startTime);
        printf("Speed rate is %d [kb/s] --\n",(int)((rxPacketSize+txPacketSize)*8/(2*(endTime-startTime)*0.001)));
        printf("----------------------\n");
